...
- **Step 17 tools cloud (2026-09-05; owner sent magicui icon-cloud link
  after «ابي قائمة بكل الtools … ونسوي فيه انيميشن حلو»; positioning
  shift: «تموضع المحاسبة عالريال مو عاجبني… وجع راس نريحه منه»).** 52
  tool icons prepared (38 mono simple-icons with currentColor + 14 official
  colour logos; Adobe Express dropped, no clean vector) in
  `apps/marketing/public/tools`, manifest `lib/tools.ts` generated by
  `scripts/tools-icons.mjs`, inventory in `docs/tools-stack-2026-09.md`.
  New `components/ToolsCloud.tsx`: canvas Fibonacci sphere written from
  scratch (Magic UI as reference): DPR-aware, pointer drag/touch, pointer
  steers rotation, hover heats mono icon to brand hex + caption, tap
  rotates icon to front and pulses the silk, group selection dims others,
  IntersectionObserver + reduced-motion pause, materialize on birth.
  Packages page section `#tools` between services stage and roadmap:
  «52 أداة تعمل لك، ولا تدير واحدة منها» + group chips + RELIEF cards
  («لا رخصة تشتريها / لا حساباً تتابعه / لا تكاملاً تركّبه / شيء واحد
  تفتحه»). Verified via Playwright desktop/mobile, no console errors.
  Pending by owner word: the «للتأكيد» tools; rewriting package items in
